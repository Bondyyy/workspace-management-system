...
ALTER TABLE CHITIETAPDUNGPGG ADD CONSTRAINT CHK_CTADPGG_Nguon
    CHECK (NguonApDung IN ('DAT_TRUOC', 'TAI_QUAY'));

ALTER TABLE CHITIETAPDUNGPGG ADD CONSTRAINT CHK_CTADPGG_Tien
    CHECK (SoTienGiam >= 0);

ALTER TABLE CHITIETAPDUNGPGG ADD CONSTRAINT CHK_CTADPGG_DoiTuong
    CHECK (MaDatCho IS NOT NULL OR MaHoaDon IS NOT NULL);
