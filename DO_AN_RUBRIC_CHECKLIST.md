...
| Xuất report sang file | Có hóa đơn PDF bằng iText. Có doanh thu CSV/PDF fallback qua `DoanhThuReportExporter`. Có báo cáo doanh thu chuyên dụng bằng JasperReports qua nút `Xuất PDF Jasper`. CSV có UTF-8 BOM, header tiếng Việt và escape đúng. | `HoaDonPDFExporter.java`, `QuanLyHoaDonForm.java`, `ThanhToanHoaDonForm.java`, `TongQuanForm.java`, `DoanhThuReportExporter.java`, `DoanhThuJasperReportExporter.java`, `bao_cao_doanh_thu.jrxml` | Đạt | Không ghi Excel nếu chưa làm Excel thật. Nếu giảng viên yêu cầu Excel, bổ sung Apache POI sau. |
...
| OOP, tổ chức lớp tốt | DTO/model riêng, util riêng, service/DAO theo module. Có helper PDF, QR, email, password, input validator, exporter doanh thu Java và exporter JasperReports. | `model/*DTO.java`, `DoanhThuReportRowDTO.java`, `HoaDonPDFExporter.java`, `DoanhThuReportExporter.java`, `DoanhThuJasperReportExporter.java`, `MaQRUtil.java`, `PasswordUtil.java`, `NguoiDungInputValidator.java` | Đạt | Trong báo cáo nêu rõ package/layer và các util/helper có test. |
...
| Unit Test JUnit | Đã có test Java thật. Bộ test hiện là unit/static test cơ bản, không cần Oracle. Có thêm test CSV exporter và test JasperReports offline. | `PasswordUtilTest.java`, `MaQRUtilTest.java`, `NguoiDungInputValidatorTest.java`, `SqlDialectStaticTest.java`, `EncodingStaticTest.java`, `SqlScriptSmokeTest.java`, `DoanhThuReportExporterTest.java`, `DoanhThuJasperReportExporterTest.java` | Đạt cơ bản | Mở rộng test nghiệp vụ/integration test nếu còn thời gian: auth service mock DAO, booking validation, Oracle integration smoke. |
...
| Điểm nâng cao/sáng tạo/triển khai thực tế | Có web app, OTP email, QR, hóa đơn PDF, doanh thu CSV/PDF, báo cáo doanh thu chuyên dụng JasperReports, scheduler, webhook thanh toán, trigger/procedure DB, RBAC, dashboard doanh thu, sơ đồ không gian. | `WebApplication.java`, `MaQRUtil.java`, `HoaDonPDFExporter.java`, `DoanhThuReportExporter.java`, `DoanhThuJasperReportExporter.java`, `WebhookThanhToanController.java`, `Database/05_triggers` | Đạt | Demo QR, PDF hóa đơn, dashboard, export doanh thu Jasper, role-based menu. |
...
| PDF doanh thu JasperReports | Có `DoanhThuJasperReportExporter`, template `bao_cao_doanh_thu.jrxml`, dữ liệu truyền bằng `JRBeanCollectionDataSource`, không query DB trực tiếp trong `.jrxml`. | Đạt cơ bản | PDF Jasper dùng text không dấu nhất quán để tránh phụ thuộc font proprietary. |
...
| `DoanhThuJasperReportExporterTest` | Kiểm tra load/compile `.jrxml` và export PDF JasperReports từ dữ liệu mẫu offline. | Report chuyên dụng bằng JasperReports. |
